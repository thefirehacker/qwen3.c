...
// ----------------------------------------------------------------------------
// Sampling visualization hook
// Emits JSONL events to stderr (or a file) showing top-N tokens, probabilities,
// nucleus membership, and the chosen token — for external dashboard consumption.

#define VIZ_TOP_N 20

void emit_sample_event(Sampler* sampler, float* raw_logits, int chosen_id, int pos) {
    int n = sampler->vocab_size;
    float temperature = sampler->temperature;
    float topp = sampler->topp;

    float* scaled = malloc(n * sizeof(float));
    ProbIndex* viz_prob = malloc(n * sizeof(ProbIndex));

    for (int i = 0; i < n; i++) scaled[i] = raw_logits[i] / temperature;
    softmax(scaled, n);

    for (int i = 0; i < n; i++) { viz_prob[i].index = i; viz_prob[i].prob = scaled[i]; }
    qsort(viz_prob, n, sizeof(ProbIndex), compare);

    float cutoff = (1.0f - topp) / (n - 1);
    int nucleus_count = 0;
    float nucleus_mass = 0.0f;
    for (int i = 0; i < n; i++) {
        if (viz_prob[i].prob >= cutoff) {
            nucleus_mass += viz_prob[i].prob;
            nucleus_count++;
            if (nucleus_mass > topp) { break; }
        } else {
            break;
        }
    }

    int show = VIZ_TOP_N < n ? VIZ_TOP_N : n;

    fprintf(stderr, "\x1e" "SAMPLE\x1e" "{\"type\":\"sample_step\",\"pos\":%d,\"temperature\":%.4f,\"topp\":%.4f,\"chosen_id\":%d,", pos, temperature, topp, chosen_id);

    char *chosen_piece = decode_token_id(chosen_id);
    fprintf(stderr, "\"chosen_piece\":\"");
    for (int i = 0; chosen_piece[i]; i++) {
        if (chosen_piece[i] == '"') fprintf(stderr, "\\\"");
        else if (chosen_piece[i] == '\\') fprintf(stderr, "\\\\");
        else if (chosen_piece[i] == '\n') fprintf(stderr, "\\n");
        else if (chosen_piece[i] == '\t') fprintf(stderr, "\\t");
        else fputc(chosen_piece[i], stderr);
    }
    free(chosen_piece);
    fprintf(stderr, "\",\"nucleus_count\":%d,\"vocab_size\":%d,\"nucleus_mass\":%.6f,\"top\":[", nucleus_count, n, nucleus_mass);

    float cumul = 0.0f;
    for (int i = 0; i < show; i++) {
        int tid = viz_prob[i].index;
        float prob = viz_prob[i].prob;
        cumul += prob;
        int in_nucleus = (cumul <= topp + prob) && (prob >= cutoff);
        float raw_logit = raw_logits[tid];
        float scaled_logit = raw_logit / temperature;

        char *piece = decode_token_id(tid);
        if (i > 0) fprintf(stderr, ",");
        fprintf(stderr, "{\"id\":%d,\"piece\":\"", tid);
        for (int j = 0; piece[j]; j++) {
            if (piece[j] == '"') fprintf(stderr, "\\\"");
            else if (piece[j] == '\\') fprintf(stderr, "\\\\");
            else if (piece[j] == '\n') fprintf(stderr, "\\n");
            else if (piece[j] == '\t') fprintf(stderr, "\\t");
            else if ((unsigned char)piece[j] < 0x20) fprintf(stderr, "\\u%04x", (unsigned char)piece[j]);
            else fputc(piece[j], stderr);
        }
        free(piece);
        fprintf(stderr, "\",\"logit_raw\":%.4f,\"logit_scaled\":%.4f,\"prob\":%.6f,\"in_nucleus\":%s}", raw_logit, scaled_logit, prob, in_nucleus ? "true" : "false");
    }
    fprintf(stderr, "]}\n");
...
// ----------------------------------------------------------------------------
// chat loop
void chat(Transformer* transformer, Tokenizer* tokenizer, Sampler* sampler, char* cli_user_prompt, char* cli_system_prompt, int think_on, int multi_turn, int tps, int ttft, int viz, TokenBuffer* tb) {
...
void error_usage() {
    ...
    fprintf(stderr, "  -v <int>    sampling viz hook: 0 = off (default), 1 = on (JSONL on stderr)\n");
    exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[]) {
    ...
    int tps = 0;         // TPS
    int ttft = 0;        // TTFT
    int viz = 0;         // sampling visualization hook
    ...
    for (int i = 2; i < argc; i+=2) {
        ...
        else if (argv[i][1] == 'v') {if ((argv[i+1][0] == '0' || argv[i+1][0] == '1') && argv[i+1][1] == '\0') {
        viz = argv[i+1][0] - '0';} else { error_usage(); } }
        else { error_usage(); }        
    }
    ...
    printf("Multi-turn = %s, thinKing = %s, tps(R) = %s, ttFt = %s, Temperature = %.2f, top-P = %.2f, Viz = %s\n", multi_turn ? "on" : "off", think_on ? "on" : "off", tps ? "on" : "off", ttft ? "on" : "off", temperature, topp, viz ? "on" : "off");
    printf("Press Enter to exit the chat\n");

    // run!
    chat(&transformer, &tokenizer, &sampler, prompt, system_prompt, think_on, multi_turn, tps, ttft, viz, &tb); 
